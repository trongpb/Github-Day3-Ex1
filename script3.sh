...
$revenue = gsub($revenue, "(\d)(?=(\d{3})+(?!\d))", "\1,") ;
...
' then  put '$release_year = int($release_year)'   \
  ...
  then head -n 1  \
  then cut -f original_title,revenue \
  then reorder -f original_title,revenue   tmdb-movies.csv  
      mlr --icsv --opprint put '  
      $original_title = string($original_title);
      $Month =   int(index($release_date, "/") == 2 ? substr($release_date, 0, 0) : substr($release_date, 0, 1));
      $Day = int(index($release_date, "/") == 2  
      ...
      ' then  put '$release_year = int($release_year)'   \
        ...
        then tail  -n 1  \
        then cut -f original_title,revenue  \
        then reorder -f original_title,revenue tmdb-movies.csv | tail -n 1  )
         # > Min_max_revenue.csv